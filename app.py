...
try:
    ...
    def build_flash_card_prompt(saved_answer: str) -> str:
        return f"""
You are a concise NSW HSC Math 4U study coach. Create one flash card from the saved answer below.
...
Format exactly:
### Front
A short recall question about the key math concept, formula, constant, figure, graph feature, or principle.
...
""".strip()
    ...
    def parse_flash_card(text: str) -> dict:
        ...
        return {
            "front": front_match.group(1).strip() if front_match else "Review this key HSC 4U Math idea.",
...
Classify the HSC 4U Math question in this image into exactly one of these types:
Multiple choice
Short answer
Calculation question
Essay
...
if mode == "Past Paper":
    list_file_path = os.path.join(BASE_ROOT, "Math_4U_List.txt")
...
st.sidebar.markdown("## 📚 Math 4U Notes")
...
current_course_key = "Math 4U" if mode == "Past Paper" else course_level
    ...
    if 0 <= q_index < len(st.session_state.image_files):
        ...
        with col2:
            ...
            if video_col.button("🎮 Video Help", key=f"video_{q_index}"):
                video_prompt = """
You are an expert NSW HSC Math teacher. Based on the image below, recommend one or two YouTube tutorial resources that help explain the concepts or topic shown.

Priority:
1. Prioritise Science Ready HSC Math tutorial videos on YouTube.
